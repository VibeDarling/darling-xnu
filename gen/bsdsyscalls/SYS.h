...
#ifdef DARLING
/* On Darling/ARM64, we call into __darling_bsd_syscall via `bl` rather than
 * executing `svc` directly. `bl` clobbers LR, so we must save+restore it
 * around the call — the macOS-style wrappers fall through to `ret` on the
 * success path without their own stp/ldp. */
